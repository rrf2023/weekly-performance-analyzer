...
from config import APP_W, APP_H
...
class WeeklyPerformanceApp:
    ...
    def load_tables(self):
        try:
            ...
            if tables:
                ...
                self.write_log(
                    f"Loaded {len(tables)} compatible table(s)."
                )
            else:
                self.table_cb.set("")
                self.week_cb.set("")
    ...
    def load_weeks(self, event=None):
        ...
        try:
            weeks = self.db.get_available_weeks(table)
    ...
    def run_all(self):
        table = self.table_cb.get().strip()
        week_start = self.week_cb.get().strip()
        ...
        save_dir = filedialog.askdirectory(
            title="Select folder to save interviewer files"
        )
        ...
        try:
            ...
            raw_df = self.db.load_week_data(table)

            if raw_df.empty:
                raise Exception(
                    "Selected table contains no data"
                )
            ...
            saved_count = self.exporter.export(
            ...
            self.write_log(
                f"DONE: created {saved_count} interviewer file(s)"
            )

            messagebox.showinfo(
                "Done",
                f"Created {saved_count} interviewer file(s)."
            )
